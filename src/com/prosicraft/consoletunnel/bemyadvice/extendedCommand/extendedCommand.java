 ...
 */
package com.prosicraft.consoletunnel.bemyadvice.extendedCommand;
...
public class extendedCommand<PluginClass extends JavaPlugin> {
    ...
    public int numArgs () {
        try {
            return args.length;
        } catch (Exception ex) {    // when args isn't set;
            return -1;
        }
    }
    ...
    public void sendMessage (CommandSender p, String msg) {
        if ( p != null ) {
            if (this.isParentSet())
                p.sendMessage(ChatColor.DARK_GRAY + "[CTunnel] " + msg);
            else
                p.sendMessage(msg);
        }
    }    
    ...
    public commandResult err (CommandSender p, String msg) {
        if ( this.isParentSet() ) {
    ...
    public commandResult suc (CommandSender p, String msg) {
        if ( this.isParentSet() ) {
            MLog.d("Inside commandResult.suc (" + String.valueOf(p) + ", " + msg + ")");
    ...
    public commandResult norm (CommandSender p, String msg) {      
        if ( this.isParentSet() ) {
